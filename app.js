...
async function loadProducts(){try{const {data,error}=await db.from('products').select('*').eq('active',true).order('created_at',{ascending:true});if(!error&&data?.length)products=data;render()}catch(e){render()}}
function render(){renderCategories();renderProducts();renderCart();document.getElementById('brandName').innerHTML=(C.storeName||'shopmate').replace(/(ai)$/,'<span>$1</span>')}
